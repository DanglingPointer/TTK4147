...
void ass_b1(void *arg){
    ...
    if (rt_sem_p(pdata->pstart_sem, TM_INFINITE)){
    ...
    if (rt_sem_v(pdata->psem))
	rt_printf("Cannot release sem\n");

    print_pri(pdata->ptask, "Released sem and finished\n");
}
...
int main() 
{
    ...
    RT_SEM my_sem, start_sem;
    if (rt_sem_create(&start_sem, "sem0", 0, S_PRIO)){
	rt_printf("Cannot create sem\n");
	return 1;
    }
    if (rt_sem_create(&my_sem, "sem1", 1, S_PRIO)){
	rt_printf("Cannot create sem\n");
	return 1;
    }
    ...
    d1.pstart_sem = d2.pstart_sem = d3.pstart_sem = &start_sem;
    ...
    usleep(100000);
    if (rt_sem_broadcast(&start_sem)){
	rt_printf("Cannot broadcast startsem\n");
	return;
    }
